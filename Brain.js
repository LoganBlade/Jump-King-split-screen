...
class Brain {
    ...
    toJSON() {
        return {
            instructions: this.instructions.map(action => ({
                isJump: action.isJump,
                holdTime: action.holdTime,
                xDirection: action.xDirection
            })),
            parentReachedBestLevelAtActionNo: this.parentReachedBestLevelAtActionNo
        };
    }

    static fromJSON(data) {
        let brain = new Brain(0, false);
        brain.instructions = data.instructions.map(a => new AIAction(a.isJump, a.holdTime, a.xDirection));
        brain.parentReachedBestLevelAtActionNo = data.parentReachedBestLevelAtActionNo || 0;
        brain.currentInstructionNumber = 0;
        return brain;
    }
